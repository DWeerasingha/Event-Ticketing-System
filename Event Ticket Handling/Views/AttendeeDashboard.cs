...
namespace Event_Ticket_Handling
{
    public partial class AttendeeDashboard : Form
    {
        ...
        private void rate_event_Click(object sender, EventArgs e)
        {
            if (dgvEvents.SelectedRows.Count > 0)
            {
                // Get the selected event's ID and Name
                int selectedEventId = Convert.ToInt32(dgvEvents.SelectedRows[0].Cells["EventID"].Value);
                string selectedEventName = dgvEvents.SelectedRows[0].Cells["EventName"].Value.ToString();

                // Open the FeedbackForm passing attendeeId, eventId, and eventName
                FeedbackForm feedbackForm = new FeedbackForm(attendeeId, selectedEventId, selectedEventName);
                feedbackForm.Show();
            }
            else
            {
                MessageBox.Show("Please select an event first to rate.", "Select Event", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
